...
int rand_self_test_block_frequency(const unsigned char *buf, size_t nbit,
                                   size_t m, double *P_value, double *Q_value)
{
...
{
    size_t n = nbit, S = 0, k;
    double pi, Vn, V, p_value, q_value;
    ...
    V = (Vn - 2.0 * n * pi * (1 - pi)) / (2.0 * pi * (1 - pi) * sqrt(2*n));
    p_value = erfc(fabs(V));
    q_value = erfc(V) / 2.0;
...
int rand_self_test_runs_distribution(const unsigned char *buf, size_t nbit, double *P, double *Q)
{
    ...
    int cur, last;
    std::vector<size_t> B_vec, G_vec;
    size_t *B = nullptr, *G = nullptr;
...
int rand_self_test_longest_run_of_ones(const unsigned char *buf, size_t nbit, bool checkone, double *P, double *Q)
{
    ...
    for (i = 0; i < N; i++) {
        ...
        run = 0;
        for (j = 0; j < M; j++) {
            if (get_bit(buf, i * M + j) == (checkone ? 1 : 0)) {
                run++;
                if (run > v_n_obs)
                    v_n_obs = run;
            } else
                                     ...
                                     size_t k, double *P, double *Q)
{
    size_t n = nbit, i, j;
    double sum = 0;
    std::vector<unsigned char> buf1_vec, buf2_vec;
    unsigned char *buf1 = nullptr, *buf2 = nullptr, *tmp;
    double p_value, q_value, V;

    buf1_vec.resize((n + 7) / 8);
    buf2_vec.resize((n + 7) / 8);
    if (buf1_vec.empty() || buf2_vec.empty())
        return 0;
    buf1 = buf1_vec.data();
    buf2 = buf2_vec.data();
...
int rand_self_test_cumulative_sums(const unsigned char *buf, size_t nbit,
                                   double *P, double *Q, bool forward)
{
    size_t n = nbit, i;
                                       ...
                                       size_t m, double *P, double *Q)
{
    ...
    for (block = m; block <= m + 1; block++) {
        if (block == 0) {
            ...
            C_vec.resize((size_t)pow(2, block), 0);
            if(C_vec.empty())
                return 0;
            C = C_vec.data();
                                     ...
                                     size_t M, double *P_value, double *Q_value)
{
    ...
    size_t *T = nullptr, *P = nullptr, *B_ = nullptr, *C = nullptr;
    std::vector<size_t> B_vec, C_vec, P_vec, T_vec;
    ...
    P_vec.resize(M);
    T_vec.resize(M);
    if(B_vec.empty() || C_vec.empty() || P_vec.empty() || T_vec.empty())
        return 0;
                                                ...
                                                size_t nbit, double *P, double *q)
{
    ...
    double sigma, V, sum, p_value, q_value, c;
    size_t *T;
    std::vector<size_t> T_vec;
                                              ...
                                              size_t nbit, double *P, double *Q)
{
    ...
    double p_value, q_value, T, N1, N0, V;
    double *m = nullptr, *X = nullptr, *wsave = nullptr;
    std::vector<double> m_vec, X_vec, wsave_vec;
    int ifac[15];

    X_vec.resize(n + 1);
    wsave_vec.resize(2 * n);
    m_vec.resize(n / 2 + 1);
    if(X_vec.empty() || wsave_vec.empty() || m_vec.empty())
        return 0;
    X = X_vec.data();
    wsave = wsave_vec.data();
    m = m_vec.data();
...
{
    int nbit = 1000000;
    std::vector<unsigned char> buf(nbit / 8);
    int fail[15] = {0};
    size_t i = 0, j, k;
    int retry = 1, res = 0;
    std::ifstream file(rand_file, std::ios::binary);
    ...
    while (i++ < 50) {
        ...
        j = 0;
        fail[j++] += rand_self_test_frequency(buf.data(), nbit, nullptr) ^ 1;
        fail[j++] += rand_self_test_block_frequency(buf.data(), nbit, 10000, nullptr) ^ 1;
        fail[j++] += rand_self_test_poker(buf.data(), nbit, 8, nullptr) ^ 1;
        fail[j++] += rand_self_test_serial(buf.data(), nbit, 5, nullptr, nullptr) ^ 1;
        fail[j++] += rand_self_test_runs(buf.data(), nbit, nullptr) ^ 1;
        fail[j++] += rand_self_test_runs_distribution(buf.data(), nbit, nullptr) ^ 1;
        fail[j++] += rand_self_test_longest_run_of_ones(buf.data(), nbit, nullptr) ^ 1;
        fail[j++] += rand_self_test_binary_derivation(buf.data(), nbit, 7, nullptr) ^ 1;
        fail[j++] += rand_self_test_self_correlation(buf.data(), nbit, 16, nullptr) ^ 1;
        fail[j++] += rand_self_test_binary_matrix_rank(buf.data(), nbit, nullptr) ^ 1;
        fail[j++] += rand_self_test_cumulative_sums(buf.data(), nbit, nullptr, nullptr) ^ 1;
        fail[j++] += rand_self_test_approximate_entropy(buf.data(), nbit, 5, nullptr) ^ 1;
        fail[j++] += rand_self_test_linear_complexity(buf.data(), nbit, 1000, nullptr) ^ 1;
        fail[j++] += rand_self_test_maurer_universal_statistical(buf.data(), nbit, nullptr) ^ 1;
        fail[j++] += rand_self_test_discrete_fourier_transform(buf.data(), nbit, nullptr) ^ 1;

        for (k = 0; k < array_size(fail); k++) {
            if (fail[k] >= 3) {
                if (--retry < 0)
                    goto end;
...
int rand_cyclical_test(const std::string &rand_file)
{
    ...
    while (i++ < 20) {
        ...
        j = 0;
        fail[j++] += rand_self_test_frequency(buf.data(), nbit, nullptr) ^ 1;
        fail[j++] += rand_self_test_block_frequency(buf.data(), nbit, 1000, nullptr) ^ 1;
        fail[j++] += rand_self_test_poker(buf.data(), nbit, 8, nullptr) ^ 1;
        fail[j++] += rand_self_test_serial(buf.data(), nbit, 5, nullptr, nullptr) ^ 1;
        fail[j++] += rand_self_test_runs(buf.data(), nbit, nullptr) ^ 1;
        fail[j++] += rand_self_test_runs_distribution(buf.data(), nbit, nullptr) ^ 1;
...
int rand_single_test(const std::string &rand_file)
{
    ...
    do {
        ...
        if (rand_self_test_poker(buf.data(), nbit, 2, nullptr) == 1)
            break;
        else
            retry--;
    } while (retry >= 0);
